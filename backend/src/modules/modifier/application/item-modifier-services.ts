...
export class ItemModifierServices {
	constructor(private readonly itemModifierRepository: ItemModifierRepository) {}
	...
	public async updateById(id: string, data: Partial<ItemModifier>): Promise<ItemModifier> {
		const itemUpdated = await this.itemModifierRepository.updateById(id, data);
		if (!itemUpdated) {
	...
	public async updateOptions(id: string, options: ItemModifierDTO["options"]): Promise<ItemModifier> {
		const itemUpdated = await this.itemModifierRepository.updateOptions(id, options);
		if (!itemUpdated) {
			throw new Error("No item updated");
		}
