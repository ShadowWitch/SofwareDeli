...
export class ItemVariantServices {
	constructor(private readonly itemVariantRepository: ItemVariantRepository) {}
	...
	public async updateById(id: string, data: Partial<ItemVariant>): Promise<ItemVariant> {
		const itemUpdated = await this.itemVariantRepository.updateById(id, data);
		if (!itemUpdated) {
