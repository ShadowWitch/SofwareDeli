...
import { useCreateItemModifier } from "~/src/hooks/item-modifier/use-create-item-modifier";
import { useUpdateItemModifier } from "~/src/hooks/item-modifier/use-update-item-modifier";
...
export function ItemModifierForm({ className, ...props }: ItemModifierFormProps) {
	...
	const onSubmit: SubmitHandler<ItemModifier> = (formData: ItemModifier) => {
		...
		createItemModifier(formDataFormatted);
		form.reset();
	};
